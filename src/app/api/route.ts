...
async function getResponse(req: NextRequest): Promise<NextResponse> {
  ...
  if (idAsNumber === 4) {
    ...
    <meta property="fc:frame:button:2:target" content="${process.env.NEXT_PUBLIC_SITE_URL}/api?id=1" />
    </head></html>`)
  }
  ...
  return new NextResponse(`<!DOCTYPE html><html><head>
    ...
    <meta property="fc:frame:post_url" content="${process.env.NEXT_PUBLIC_SITE_URL}/api?id=${nextId}" />
  </head></html>`)
}
